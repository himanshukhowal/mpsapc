...
export class PaymentService {
  ...
  protected convertDateFromClient(payment: IPayment): IPayment {
    const copy: IPayment = Object.assign({}, payment, {
      dateCreated: payment.dateCreated && payment.dateCreated.isValid() ? payment.dateCreated.format(DATE_FORMAT) : undefined,
      dateModified: payment.dateModified && payment.dateModified.isValid() ? payment.dateModified.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((payment: IPayment) => {
        payment.dateCreated = payment.dateCreated ? moment(payment.dateCreated) : undefined;
        payment.dateModified = payment.dateModified ? moment(payment.dateModified) : undefined;
      });
    }
    return res;
  }
